/* SiteCatalyst code version: H.15.1.20091104
...
if(window.location.hostname.match(/spur\.jp/) ){
	var s_account="shshaspurjp"
}
else if(window.location.hostname.match(/shueisha\.co\.jp/) ){
	var s_account="shshaspurjp"
}
else{
	var s_account="shshaspurjpdev"
}
...
if( document.URL.match(/ne\.jp/) ){ s.cookieDomainPeriods="3" }
/* Conversion Config */
s.currencyCode="JPY"
...
s.linkInternalFilters="javascript:,spur.jp"
s.linkLeaveQueryString=false
s.linkTrackVars="events,prop21,prop22,prop23,prop24,eVar21,eVar22,eVar23,eVar24"
...
function s_doPlugins(s) {
	...
	/* getEngineAndKW */
	var search_word = getEngineAndKW(document.URL, document.referrer, 'cid');

	/* Add calls to plugins here */
	s.campaign=s.getQueryParam('cid');

	s.hier1  = s.pageName;
	s.hier2  = window.location.pathname;

	s.prop1 = s.eVar1 = s.getValOnce(s.campaign,'s_campaign',0);
	if(search_word){ s.prop2 = s.eVar2 = search_word.slice(0,20); }
	s.prop3 = s.eVar3 = s.campaign;
	s.prop4 = s.eVar4 = window.location.protocol +'//'+ window.location.hostname + window.location.pathname;
	s.prop5 = s.eVar5 = s.getNewRepeat();
	s.prop6 = s.eVar6 = s.getTimeParting('h','+8',currentYear);
	s.prop7 = s.eVar7 = s.getTimeParting('d','+8',currentYear);
	s.prop8 = s.getDaysSinceLastVisit();
	s.eVar8 = s.getAndPersistValue(s.prop8,'s_delv',0);

	/* get cookie */
	var mycookie = document.cookie+"/";
	var mycookieflg = mycookie.indexOf("sc_spuruser=");
	if( mycookieflg != -1){
		var txtlen = mycookie.indexOf(";", mycookieflg);
		var txt = mycookie.substring(mycookieflg+"sc_spuruser=".length, txtlen);
		txt = decodeURI(txt);
		var ever_array = txt.split('%26');
		s.prop21 = s.eVar21 = ever_array[0];
		s.prop22 = s.eVar22 = ever_array[1];
		s.prop23 = s.eVar23 = ever_array[2];
		s.prop24 = s.eVar24 = ever_array[3];
	}

	/* pathing */
	if(s.pageName){
		s.prop41 = s.campaign +':'+ s.pageName.slice(0,15);
		s.prop42 = s.prop5    +':'+ s.pageName.slice(0,15);
	}
...
function getEngineAndKW(u,r,ps){
	if(isPaidSearch(u,ps)!=1){var e=getSearchEngine(r);
		if(e){var k=getSearchTerm(r,getEngineParam(e));return (e+"|"+k);}
	}
	return null;
}
function getEngineAndKW4Pathing(u,r,ps){
	if(isPaidSearch(u,ps)!=1){var e=getSearchEngine(r);
...
function getSearchEngine(r){
	if(r.indexOf('?')>-1){
		...
		if(     d.indexOf('google.com')   >-1 && q.match(/^q=|&q=/) ){e="G_US";}
		else if(d.indexOf('google.co.jp') >-1 && q.match(/^q=|&q=/) ){e="G_JP";}
		else if(d.indexOf('yahoo.com')    >-1 && q.match(/^p=|&p=/) ){e="Y_US";}
		else if(d.indexOf('yahoo.co.jp')  >-1 && q.match(/^p=|&p=/) ){e="Y_JP";}
		else if(d.indexOf('bing.com')     >-1 && q.match(/^q=|&q=/) ){e="B_US";}
		else if(d.indexOf('msn.com')      >-1 && q.match(/^q=|&q=/) ){e="M_US";}
		else if(d.indexOf('msn.co.jp')    >-1 && q.match(/^q=|&q=/) ){e="M_JP";}
	}
	return e;
}

function getEngineParam(e){
	var p="q";if(e.indexOf('Y_')>-1){p="p";}return p;
}
