/* SiteCatalyst code version: H.20.3. http://www.omniture.com */

var s_account="sonyericssoncomjapanprod";

/************************** CONFIG SECTION **************************/
var s_omni=new Object();
s_omni.sHost = location.host.toLowerCase();
s_omni.sPath = location.pathname.toLowerCase();
if(!s_omni.sHost.match(/sonyericsson\.co\.jp$/)) {
  s_account="sonyericssoncomjapandev";
}
...
s.visitorNamespace="sonyericsson";
s.trackingServer="metrics.sonyericsson.com";
s.trackingServerSecure="metrics.extranet.sonyericsson.com";
s.dc="112";
/* Conversion Config */
s.currencyCode="JPY";
/* Link Tracking Config */
if(s_omni.sHost.indexOf("sonyericsson.co.jp") > -1){
  s.trackDownloadLinks=true;
  s.trackExternalLinks=true;
  s.trackInlineStats=true;
  s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";
  s.linkInternalFilters="javascript:,sonyericsson.co.jp";
  /* Plugin Config */
  s.usePlugins=true;
}else{
...
function s_doPlugins(s) {
    ...
    // flash
    s.detectRIA('s_ria','prop10','','10','','1');

    //download
    s.objDlh = s.downloadLinkHandler();
    if (s.objDlh) {
        s.linkTrackVars="prop7,prop8,eVar7,events";
        s.linkTrackEvents="event4";
        s.prop7 = s.eVar7 = s.ojbDlh;
        s.prop8 = s.pageName;
        s.events = "event4";
    }

    //partner and support
    s.objLh = s.linkHandler("www_nttdocomo_co_jp|www_au_kddi_com|mb_softbank_jp|www_jp_sonystyle_com|faq.sonyericsson.co.jp");
    if (s.objLh) {
        s.linkType = "e";
        s.linkName = s.objLh;
        if(s.channel == "sejp : prod") {
            s.linkTrackVars = "eVar6,events";
            s.linkTrackEvents = "event6";
            s.events = "event6";
            s.eVar6 = s.objLh.match(/(docomo|au_kddi|sonystyle)/)[1];
        } else {
            if (s.objLh.match(/www_nttdocomo_co_jp/)) {
                s.linkTrackVars = "eVar2,events";
                s.linkTrackEvents = "event3";
                s.events = "event3";
                s.eVar2 = "docomo support";
            } else if (s.objLh.match(/www_au_kddi_com/)) {
                s.linkTrackVars = "eVar2,events";
                s.linkTrackEvents = "event3";
                s.events = "event3";
                s.eVar2 = "au support";
            } else if (s.objLh.match(/mb_softbank_jp/)) {
                s.linkTrackVars = "eVar2,events";
                s.linkTrackEvents = "event3";
                s.events = "event3";
                s.eVar2 = "softbank support";
            } else if (s.objLh.match(/faq\.sonyericsson\.co\.jp/)) {
    ...
    // non-redirect link
    s.objElh = s.exitLinkHandler();
    if (s.objElh) {
        if(s.objElh.indexOf("kddi") > -1) {
            s.linkTrackVars = "eVar6,events";
            s.linkTrackEvents = "event6";
            s.events = "event6";
            s.eVar6 = "au_kddi";
        } else if(s.objElh.indexOf("docomo") > -1) {
            s.linkTrackVars = "eVar6,events";
            s.linkTrackEvents = "event6";
            s.events = "event6";
            s.eVar6 = "docomo";
        } else if(s.objElh.indexOf("sonystyle") > -1) {
            s.linkTrackVars = "eVar6,events";
            s.linkTrackEvents = "event6";
            s.events = "event6";
            s.eVar6 = "sonystyle";
        }  else if (s.objElh.match(/ssl\.okweb3\.jp/)) {
            s.linkTrackVars = "eVar2,events";
            s.linkTrackEvents = "event3";
            s.events = "event3";
            s.eVar2 = "mail support";
        }
    }

    if (document.title == "Not Found | Sony Ericsson") {
        s.pageType = "errorPage";
        s.pageName = "404 : <" + location.href + ">";
        s.channel = s.prop1 = s.prop2 = "";
    }
...
if(s_omni.sHost.indexOf("sonyericsson.co.jp") > -1) {
     ...
     * */
    s_omni.sSiteType = "";
    if (s_omni.sHost == "www.sonyericsson.co.jp" || s_omni.sHost == "sonyericsson.co.jp"){
        s_omni.sSiteType = "sejp";
        s.server = s_omni.sHost;
    } else if (s_omni.sHost == "www.fan.sonyericsson.co.jp") {
        s_omni.sSiteType = "sejp/fan";
        s.server = s_omni.sHost;
    }
    s_omni.sPath = s_omni.sPath.replace(/\/index\.html$/, "\/");
    s_omni.sPath = s_omni.sPath.replace(/\/$/, "/top");
    s_omni.sPath = s_omni.sPath.replace(/campaign/g,"cp");
    s_omni.sPath = s_omni.sPath.replace(/company/g,"comp");
    s_omni.sPath = s_omni.sPath.replace(/download/g, "dl");
    s_omni.sPath = s_omni.sPath.replace(/product/g,"prod");
    s_omni.sPath = s_omni.sPath.replace(/accessories/g,"acc");
    s_omni.sPath = s_omni.sPath.replace(/docomo/g,"d");
    s_omni.sPath = s_omni.sPath.replace(/support/g,"supp");
    s_omni.sPath = s_omni.sPath.replace(/software/g,"soft");
    s_omni.sPath = s_omni.sPath.replace(/whatsnew/g,"wn");

    s_omni.aPageName = (s_omni.sSiteType + s_omni.sPath).split("/");
    for (s_omni.i = 0; s_omni.i < s_omni.aPageName.length; s_omni.i++){
        if(s_omni.aPageName[s_omni.i]){
            if (s_omni.i === 0){
                s.pageName = s.channel = s.prop1 = s.prop2 =  s_omni.aPageName[0];
            } else if (s_omni.i == 1) {
                s.pageName += ":" + s_omni.aPageName[1];
                s.channel += ":" + s_omni.aPageName[1];
                s.prop1 += ":" + s_omni.aPageName[1];
                s.prop2 += ":" + s_omni.aPageName[1];
            } else if (s_omni.i == 2) {
                s.pageName += ":" + s_omni.aPageName[2];
                s.prop1 += ":" + s_omni.aPageName[2];
                s.prop2 += ":" + s_omni.aPageName[2];
            } else if (s_omni.i == 3){
                s.pageName += ":" + s_omni.aPageName[3];
                s.prop2 += ":" + s_omni.aPageName[3];
            } else if (s_omni.i >= 4){
                s.pageName += ":" + s_omni.aPageName[s_omni.i];
            }
        }
    }
    s.pageName = s.pageName.replace(/:/g, " : ");
    s.channel = s.channel.replace(/:/g, " : ");
    s.prop1  = s.prop1.replace(/:/g, " : ");
     ...
     * */
    // search
    if (location.pathname.indexOf("search.cgi") > -1 &&
        document.forms[0] && document.forms[0].q && document.forms[0].q.value){
        s.prop5 = s.eVar5 = document.forms[0].q.value;
        s.prop11 = s.getPreviousPage();
        s.events = s.apl(s.events,"event5",",",2);

        s_omni.d = document.getElementById("contentsArea");
        s_omni.res = s_omni.d.getElementsByTagName("div");
        for (s_omni.i = 0; s_omni.i < s_omni.res.length; s_omni.i++) {
            if (s_omni.res[s_omni.i].getAttribute("className") == "res_noresult" ||
                s_omni.res[s_omni.i].getAttribute("class") == "res_noresult"){
                s.prop5 = "null:" + s.prop5;
                s.eVar5 = s.prop5;
                break;
            }
        }
    ...
    // support
    if (s.channel == "sejp : supp") {
        s_omni.aSupp = location.pathname.split("/");
        if(s_omni.aSupp[2] && s_omni.aSupp[2].indexOf(".") < 0){
            s.prop9 = s_omni.aSupp[2];
        }
    }
    //products
    if (s.channel == "sejp : prod") {
        s_omni.aProd = location.pathname.split("/");
        if(s_omni.aProd[3] && s_omni.aProd[3].indexOf(".") < 0){
            s.prop14 = s_omni.aProd[3];
            s.products = s_omni.aProd[2] + ";" + s_omni.aProd[3];
 ...
 * track blog parts
 * */
s.blogParts = function(blog){
    s.un = "sonyericssoncomjapanprod";
    s.linkTrackVars="eVar31,eVar32";
    s.eVar31=blog;
    s.eVar32=location.host;
    s.tl(true, "o", blog);
};
